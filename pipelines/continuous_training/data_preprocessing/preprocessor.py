...
class Preprocessor:
    ...
    def _transform_with_robust_scaler(
        ...
        for feature in features:
            ...
            robust_scaler = robust_scaler.fit(x_train[[feature]])
            robust_scalers[feature] = robust_scaler

            x_train[feature] = robust_scaler.transform(x_train[[feature]])
            x_val[feature] = robust_scaler.transform(x_val[[feature]])
        

        # TODO: robust_scalers 딕셔너리를 self._encoder_path에 robust_scaler.joblib 이름으로 저장
        joblib.dump(robust_scalers, 
                    os.path.join(self._encoder_path,
